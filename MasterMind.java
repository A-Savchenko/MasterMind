...
public class MasterMind {
	...
	{
		Prompt p = new Prompt();
		p.getString("Hit the Enter key to start the game ");
		makeACode();
		playerMethod();
	}

	public void makeACode()
	{
		for(int i = 0; i < 4; i++)
		{
			master.setPeg(i,(char)((int)(Math.random()*5+1)+64));
		}
		System.out.println();
	}
	public void playerMethod()
	{
		...
		{
			for(int y = 0; y < PEGS_IN_CODE;y++)
			{
				guesses[x] = new PegArray(4);
				guesses[x].setPeg(y,'X');
			}
		}
		for(int i = 0; i < MAX_GUESSES; i++)
		{
			...
			while(fr)
			{
				guess = p.getString("Enter the code using (A,B,C,D,E,F). For example, ABCD or abcd from left-to-right ");
				System.out.println();
				guess = guess.toUpperCase();
				if(guess.length() == 4)
				{
					for(int x = 0; x < PEGS_IN_CODE;x++)
					{
						if(guess.charAt(x)<65||guess.charAt(x)>70)
						{
			...
			for(int x = 0; x < PEGS_IN_CODE; x++)
			{
				masterWord += master.getPeg(x).getLetter();
				guesses[i].setPeg(x,guess.charAt(x));
			}
			guesses[i].getExactMatches(master);
			guesses[i].getPartialMatches(master);

			int exact = guesses[i].getExact();
			if(exact == 4) reveal = true;
			if(reveal) {
				printBoard();
				System.out.println("Nice work! You found the master code in "+(i+1)+" guesses. ");
				i = MAX_GUESSES;
			}
		...
		if(!reveal && loss)
		{
			System.out.println("Nice try, the word was "+ masterWord);
		}
	}
}
